...
role | 否 | 用户和设备的订阅关系，0：管理员，1：普通用户
...
product_id | 是 | 产品ID
device_id | 是 | 设备ID
type | 否 | 固件升级任务类型, 默认为wifi, 数字类型 默认为1, 值请看[设备固件类型](#firmware_type)
...
product_id | 是 | 产品ID
device_id | 是 | 设备ID
type | 否 | 固件升级任务类型,数字类型 默认为1, 值请看[设备固件类型](#firmware_type)
identify | 否 | 用来定位多MCU或多子设备的情况, 数字, type为mcu或多子设备时必传, 默认为0, 新增参数
